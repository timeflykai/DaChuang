...
def get_news(url):
    ...
    content_html=str(main_part.find_all("div",class_="TRS_Editor")[1]).replace("./",path)
    News={"title":title,"date":date,"content":content_html}
    with open("./static/news/2.json","w",encoding="utf-8")as f:
...
def get_newslist(url):
    ...
    for item in main_part:
        ...
        newslist.append(dic)
    with open("./static/news/news_list.json","w",encoding="utf-8")as f:
